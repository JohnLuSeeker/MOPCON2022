...
//        }
//        listMonths = tempListMonths.reversed().toList()
//    }

    fun getMonth(offset: Int = 0, firstDayOfWeek: DayOfWeek): Month {
        val dayOfWeekOffset = 7 % 7
        val now = org.joda.time.LocalDate.now().plusMonths(offset)
        val from = now.withDayOfMonth(1)
        val to = now.plusMonths(1).withDayOfMonth(1).plusDays(-1)
        Log.d("Test week first", from.plusDays((dayOfWeekOffset -from.dayOfWeek) % 7).toString())
        Log.d("Test week last", to.plusDays((dayOfWeekOffset -to.dayOfWeek) % 7).toString())
        val yearMonth = YearMonth.from(LocalDate.now()).plusMonths(offset.toLong())
        val numberWeeks = org.joda.time.LocalDate.now().plusMonths(offset).getNumberWeeks(7)
        val listWeekItems = mutableListOf<Week>()
        for (week in 0 until numberWeeks) {
            listWeekItems.add(
                Week(
        ...
        return Month(yearMonth, listWeekItems)
    }
